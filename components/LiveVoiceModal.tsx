...
export const LiveVoiceModal: React.FC<LiveVoiceModalProps> = ({ isOpen, onClose, grade, subject }) => {
  ...
  const connect = useCallback(async () => {
    try {
      ...
      try {
        ...
        setErrorMessage("تعذر الوصول للميكروفون. يرجى التأكد من توصيله والسماح بالوصول.");
        setStatus('error');
        return;
      }
      ...
      const systemInstruction = `
        ...
        مهمتك: مساعدة الطالب في المذاكرة.
        ...
        callbacks: {
          ...
          onerror: (err) => {
            ...
            if (mountedRef.current) {
                setErrorMessage("حدث خطأ في الاتصال بالخدمة.");
                setStatus('error');
            }
          }
        }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Safe cleanup for audio context
      ...
      <div className="flex flex-col items-center gap-8 w-full max-w-md">
         {status === 'connecting' && (
           <div className="flex flex-col items-center gap-4 text-indigo-200">
              ...
              <p className="text-lg font-medium">جاري الاتصال...</p>
           </div>
         )}
         ...
         {status === 'error' && (
           <div className="flex flex-col items-center gap-4 text-red-300">
              <PhoneOff size={48} />
              <p className="text-lg font-medium text-center">{errorMessage || "تعذر الاتصال."}</p>
