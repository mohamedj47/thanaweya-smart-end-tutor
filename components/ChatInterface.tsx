...
import { Send, ChevronRight, HelpCircle, FileText, Lightbulb, Bot, List, Printer, Mic, Camera, Paperclip, X, AudioLines, StopCircle, Sparkles } from 'lucide-react';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ grade, subject, onBack }) => {
  ...
  useEffect(() => messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }), [messages, attachment, isLoading]);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] chat-container relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {messages.map((msg) => (
          <div key={msg.id}>
             {msg.attachment && <div className="flex justify-end mb-1"><span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded-md">📎 {msg.attachment.name || 'مرفق'}</span></div>}
      ...
      <div className="p-4 bg-white border-t border-slate-100 input-area">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-2 flex items-end shadow-inner focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
          <div className="flex gap-1 pb-1 px-1">
             <button onClick={() => setIsLiveMode(true)} className="p-2 rounded-full bg-indigo-100 text-indigo-600"><AudioLines size={18} /></button>
          ...
          <textarea
            ...
            placeholder={isRecording ? "جاري التسجيل..." : "اكتب سؤالك هنا..."}
