...
export const FloatingTools: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Chrome/Android requires this event listener
  ...
  const handleReadPage = () => {
    ...
    // 3. Clean Text (Remove Markdown, symbols, URLs)
    const cleanText = textToRead
      .replace(/[*#`_\-]/g, '') // Remove Markdown chars
      .replace(/https?:\/\/\S+/g, '') // Remove URLs
      .trim();

    // 4. Detect Language (Robust Count-Based)
    const arabicCount = (cleanText.match(/[\u0600-\u06FF]/g) || []).length;
    const latinCount = (cleanText.match(/[a-zA-Z]/g) || []).length;
    
    // Determine target language based on character count
    const isArabic = arabicCount >= latinCount;
    ...
    // 6. FORCE VOICE SELECTION (Crucial for Android/Chrome)
    if (voices.length > 0) {
        let selectedVoice = voices.find(v => v.lang === targetLang);

        if (!selectedVoice) {
            // Fuzzy match (e.g. 'ar' or 'en')
        ...
        const shortCode = targetLang.split('-')[0];
        const googleVoice = voices.find(v => v.lang.startsWith(shortCode) && v.name.includes("Google"));
